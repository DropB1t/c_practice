...
int find(const char nomedir[], const char nomefile[]) {
  ...
  if (chdir(nomedir) == -1) {
    if (!quiet) fprintf(stderr,"Impossibile entrare nella directory %s\n", nomedir);
    return 0;
  }
  DIR * dir;
  if ((dir=opendir(".")) == NULL) {
    if (!quiet) fprintf(stderr,"Errore aprendo la directory %s\n", nomedir);
    return -1;
  } else {
    struct dirent *file;
	...
	if ( !isdot(file->d_name) ) {
	  if (find(file->d_name,nomefile) != 0) {
	    ...
	    if (chdir("..")==-1) {
	      fprintf(stderr,"Impossibile risalire alla directory padre.\n");
	      return -1;
	    }
	  }
	} 
      } else {
	if (strlen(file->d_name) == strlen(nomefile) &&
	    strncmp(file->d_name, nomefile, strlen(nomefile)) == 0) {
	  char* buf = cwd();
